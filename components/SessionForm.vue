...
async function handleSubmit() {
    ...
    try {
        ...
        // Validate using a schema without id (id is generated by the store)
        const inputSchema = SessionSchema.omit({ id: true })
        const result = inputSchema.safeParse(sessionData)
        ...
        if (props.isEditing && props.session?.id) {
            const storeResult = store.updateSession(props.session.id, result.data)
            if (!storeResult.success) return
            savedSession = storeResult.data
        } else {
            const storeResult = store.addSession(result.data)
            if (!storeResult.success) return
            savedSession = storeResult.data
        }
